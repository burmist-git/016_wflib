...
waveform::waveform(const Double_t *wfT, const Double_t *wfA, Int_t nPoints, Int_t nPointsBaseLine){
  _nSplinePoints = 0;
  _nPoints = nPoints;
...
waveform::waveform(const Double_t *wfT, const Double_t *wfA, Int_t nPoints, Int_t nPointsBaseLine, Int_t nSplinePoints){
  if(nSplinePoints<=0){
...
waveform::waveform(const Double_t *wfT, const Double_t *wfA, Int_t nPoints, Int_t nPointsBaseLine, Int_t nSplinePoints, Int_t sigPolarity){
  if(nSplinePoints<=0){
    cout<<"  ERROR --> nSplinePoints <= 0"<<endl
	<<"            nSplinePoints  = "<<nSplinePoints<<endl;
    assert(0);
...
void waveform::normalizeWF(double normVal){
  if(normVal>0.0){
    for(int i = 0;i<_nPoints;i++)
...
Double_t waveform::Get_wfAi(int i){
  if(i>=0 && i<_nPoints)
    return _wfA[i];
  assert(0);
  return -999.0;
...
Double_t waveform::Get_wfTi(int i){
  if(i>=0 && i<_nPoints)
    return _wfT[i];
  assert(0);
  return -999.0;
...
void waveform::alignInTimeOfMaximum(double tVal){
  double dT = _wfT[_i_amax] - tVal;
  for(Int_t i = 0;i<_nPoints;i++)
    _wfT[i] = _wfT[i] - dT;
}

Double_t waveform::findBaseLineAmpl(Int_t np){
  Double_t abasel = 0;
  for(int i = 0;i<np;i++)
    abasel += _wfA[i];
  return abasel/np;
...
Double_t waveform::findBaseLineAmpl(Int_t np, const Double_t *wfA){
  Double_t abasel = 0;
  for(int i = 0;i<np;i++)
    abasel += wfA[i];
  return abasel/np;
...
void waveform::Draw(TString key_lineOn){
...
void waveform::findPar( Double_t tLeft, Double_t tRight, Double_t  vVal, Double_t cfdValForTiming){
  _chargeWindow_tLeft = tLeft;
  _chargeWindow_tRight = tRight;
  _vVal = vVal;
  double timeCFDt;       //cfdValForTiming;
  ...
  for(Int_t i = i_amax; i>0 ; i--){
    if(_wfA[i]>=amax*cfdValForTiming && _wfA[i-1]<amax*cfdValForTiming){
      timeCFDt = linInterpol(amax*cfdValForTiming,
			     _wfA[i],_wfA[i-1],
			     _wfT[i],_wfT[i-1]);
      break;
    }
